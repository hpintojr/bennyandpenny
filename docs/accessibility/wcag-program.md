...
3. Focus remains visible and is never obscured.
...
9. Public accessibility controls never create a dead end: a minimized control always leaves a visible, keyboard-accessible restore path.
...
Use the approved universal-access person-in-circle asset:

```txt
public/images/accessibility-universal-icon.svg
```

- It replaces the previous thin-line person, which was ambiguous at small sizes.
- It is rendered as a CSS mask so the same SVG inherits the active theme color.
- The SVG uses an optimized square crop and internal padding so it reads correctly in the launcher.
- The icon is decorative inside the launcher button; the button provides the accessible name.
- Do not redraw, stretch, or replace it with a thin-line stick figure.

### Placement and visibility

- Render on every public page, fixed at the lower-left viewport edge with safe-area spacing.
- Use a native button with the approved icon, a programmatic name of **Open accessibility preferences**, and a minimum 44 by 44 CSS pixel target.
- Use the approved default theme: cream shell, deep-teal primary button, white icon; deep-teal icon on the cream restore tab.
...
- The launcher works with mouse, touch, Tab, Enter, and Space.
- The native dialog traps focus while open and uses the browser modal layer so background content is inactive.
- Respect `prefers-reduced-motion` and the site pause-motion setting. No essential action depends on animation.
- Hide the launcher in print styles only.

### Appearance and non-interference

- Verify contrast for the icon, button, tooltip, border, and focus indicator.
- Keep the dock below the modal dialog layer and above normal page content.
- Do not cover cookie banners, chat controls, vital form actions, emergency/help controls, cart controls, or checkout controls.
- Reassess dock position whenever a persistent third-party widget is added.
...
- Automated browser tests cover opening, minimizing, restoring, dialog controls, icon asset presence, and persistence after reload.
- Screen-reader tests announce launcher, minimize/restore controls, dialog title, and state changes clearly.
...
- Floating, collapsible native accessibility launcher using the approved universal-access icon with local-device persistence and a recoverable restore tab.
...
When the Benny & Penny's Adventures site is updated, copy the behavioral standard above—not merely the appearance. The Adventures implementation uses the same approved icon asset/visual rules, native controls, footer trigger, Accessibility page, recoverable minimized state, keyboard/focus contract, and route-by-route tests for store, book detail, cart, checkout, account, library, gifting, orders, and customer-support journeys.
