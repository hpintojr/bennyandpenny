# Work With Benny & Penny's

> Start a conversation about the next platform, integration, brand, or publishing project.

Canonical page: https://bennyandpenny.com/work-with-us

Benny & Penny's takes on a small number of aligned engagements across software, business, brand, web, and publishing.

## Engagement types

- Software architecture and custom platform builds
- API integration, CRM integration, telephony, systems, and business intelligence
- Business consulting and development
- Brand, visual identity, marketing, and web development
- Go-to-market and launch systems
- Publishing, imprint, and production-pipeline projects

## What to expect

Hamilton listens for the heart of the project first, then provides a clear, honest plan for an aligned build.

## Contact

Contact form: https://bennyandpenny.com/work-with-us
Email: hello@bennyandpenny.com
